-- Dev veteran gets a real review history (2026-08-05).
--
-- seed_dev_veteran built card_fsrs_state directly and never wrote review_logs or
-- study_events. That was invisible while Progress showed Sessions and Streaks
-- sourced elsewhere; the All-Time refactor to Reviews / Days active / Time
-- invested reads the log for all three, and the scenario that is supposed to
-- represent five years of study reported ten reviews across one day.
--
-- The function now reconstructs the log from the states it already writes, so a
-- reseed produces a coherent account rather than a plausible-looking front page
-- with nothing behind it. Dev-only: the is_dev / dev-veteran@ gate is unchanged.
...
begin
  ...
  -- One session per day the log says they studied. Only the recent ones carry a
  -- duration: durations have existed since 2026-07-30, so a fixture where every
  -- session is timed would never exercise the estimate path that real accounts
  -- take. 6.5s/card sits inside commit_quiz_session's 1s-10min sanity bounds.
  insert into public.study_events (user_id, event, props, occurred_at)
  select v_uid, 'quiz_completed',
         jsonb_build_object('cards', d.cnt)
           || case when d.day_ts >= now() - interval '45 days'
                   then jsonb_build_object('duration_ms', (d.cnt * 6500)::bigint)
                   else '{}'::jsonb end,
         d.day_ts
  from (
